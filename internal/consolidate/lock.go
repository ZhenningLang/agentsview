...
var ErrLocked = errors.New("consolidate: lock held by another instance")

// lockFile is the single-flight lock filename created inside the staging dir.
// A live agentsview instance plus a CLI invocation can race to consolidate the
// same staging dir; the O_EXCL pid file makes the loser skip its cycle.
...
// AcquireLock tries to take the staging consolidation lock by exclusively
// creating <stagingDir>/.consolidate.lock with the current pid. It returns
// ErrLocked when the file already exists (another holder). A stale lock whose
// recorded pid is no longer running is reclaimed so a crashed run does not wedge
// the worker forever.
func AcquireLock(stagingDir string) (*Lock, error) {
	if err := os.MkdirAll(stagingDir, 0o700); err != nil {
		return nil, fmt.Errorf("creating staging dir for lock: %w", err)
	}
	path := filepath.Join(stagingDir, lockFile)
	...
	if stalePid, dead := lockIsStale(path); dead {
